...
import {
  Package, CheckCircle, Clock, XCircle, Search, MessageCircle,
  ExternalLink, ChevronDown, ChevronUp, ShoppingBag, TrendingUp,
} from "lucide-react";
...
const OrderManager = () => {
  ...
  const updateStatus = async (orderId: string, newStatus: string) => {
    ...
    else { toast.success(`অর্ডার ${newStatus} করা হয়েছে`); fetchOrders(); }
  };

  const toggleExpand = (orderId: string) => {
    if (expandedOrder === orderId) setExpandedOrder(null);
    else { setExpandedOrder(orderId); fetchOrderItems(orderId); }
  };
  ...
  const filtered = orders.filter((o) => {
    const matchSearch = o.customer_name.toLowerCase().includes(searchTerm.toLowerCase()) || o.trx_id.toLowerCase().includes(searchTerm.toLowerCase()) || o.customer_phone.includes(searchTerm);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="নাম, ফোন বা TrxID দিয়ে খুঁজুন..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" variant={statusFilter === "all" ? "default" : "outline"} className="rounded-full text-xs" onClick={() => setStatusFilter("all")}>সব ({orders.length})</Button>
          {statusOptions.slice(0, 4).map((s) => (
            <Button key={s.value} size="sm" variant={statusFilter === s.value ? "default" : "outline"} className="rounded-full text-xs" onClick={() => setStatusFilter(s.value)}>
          ...
          {filtered.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-card rounded-xl border border-border/50 overflow-hidden">
                <div className="p-4 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => toggleExpand(order.id)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="font-semibold text-foreground">{order.customer_name}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusOpt.color}`}>{statusOpt.labelBn}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      {order.customer_whatsapp && (
                        <a href={getWhatsAppLink(order.customer_whatsapp, order.customer_name)} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 rounded-full bg-emerald-100 hover:bg-emerald-200 transition-colors">
                          <MessageCircle className="h-4 w-4 text-emerald-700" />
                        </a>
                      )}
                      {order.customer_telegram && (
                        <a href={`https://t.me/${order.customer_telegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 rounded-full bg-blue-100 hover:bg-blue-200 transition-colors">
                  ...
                  <div className="border-t border-border p-4 space-y-4 bg-muted/20 animate-in fade-in duration-200">
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {Number(order.cash_on_delivery) > 0 && <p><span className="text-muted-foreground">ক্যাশ অন ডেলিভারি:</span> ৳{order.cash_on_delivery}</p>}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {statusOptions.map((s) => (
                          <Button key={s.value} size="sm" variant={order.status === s.value ? "default" : "outline"} className="rounded-full text-xs" onClick={() => updateStatus(order.id, s.value)} disabled={order.status === s.value}>
                            {s.labelBn}
                          </Button>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
                      {order.customer_whatsapp && (
                        <a href={getWhatsAppLink(order.customer_whatsapp, order.customer_name)} target="_blank" rel="noopener noreferrer">
                      ...
                      {order.customer_telegram && (
                        <a href={`https://t.me/${order.customer_telegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer">
